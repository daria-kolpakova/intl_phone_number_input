...
class _InputWidgetState extends State<InternationalPhoneNumberInput> {
  ...
  void phoneNumberControllerListener() {
    if (this.mounted) {
      ...
      getParsedPhoneNumber(
        ...
        dealCode: this.country!.dialCode!,
  ...
  Future<String?> getParsedPhoneNumber(
      {required String phoneNumber, required String dealCode, required String isoCode}) async {
    try {
      bool? isValidPhoneNumber = await PhoneNumberUtil.isValidNumber(phoneNumber: phoneNumber, isoCode: isoCode);

      if (isValidPhoneNumber!) {
        return await PhoneNumberUtil.normalizePhoneNumber(dealCode: dealCode, phoneNumber: phoneNumber);
      }
    } on Exception {
      return null;
    }
